...
const createReport = ({ hasTrace, hasTraceSet = false, hasTraceChildren = false, addFn, condition = {} }) => {
    if (hasTrace) {
...
const composeHandler = ({ path, method, hooks, validator, handler, handleError, definitions, schema, onRequest, config, reporter }) => {
    ...
    const hasTraceChildren = hasTraceSet &&
        traceLiterals.some((fn) => fn.includes('children') || isContextPassToFunction(fn));
    ...
    return createHandler({
        ...
        signCookie: cookie_signature_1.sign
    });
};
...
const composeGeneralHandler = (app) => {
    ...
    const report = createReport({
        hasTrace,
        hasTraceChildren: hasTrace &&
            traceLiteral.some((x) => x.includes('children') || isContextPassToFunction(x)),
        condition: {
